...
# Res50
class FcaNet50(nn.Module):
    def __init__(self, pretrained=True):
        super(FcaNet50, self).__init__()
...
class Fca(nn.Module):
    def __init__(self, pretrained=True):
        ...
        self.own_reslayer_1 = make_res_layer(FcaBottleneck, 64, 64, 3, stride=1)
        self.own_reslayer_1.load_state_dict(res.layer1.state_dict(), False)
        self.own_reslayer_2 = make_res_layer(FcaBottleneck, 256, 128, 4, stride=1)
        self.own_reslayer_2.load_state_dict(res.layer2.state_dict(), False)
        self.own_reslayer_3 = make_res_layer(FcaBottleneck, 512, 256, 6, stride=1)
        self.own_reslayer_3.load_state_dict(res.layer3.state_dict(), False)

    def forward(self, x):
        x = self.frontend(x)

        x = self.own_reslayer_1(x)
